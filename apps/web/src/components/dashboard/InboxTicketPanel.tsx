...
export default function InboxTicketPanel({ conversationId, agentId, ticketId, ticketNumber, suggestedSubject, operators, canRetryDelivery }: Props) {
  ...
  if (ticket || ticketId) {
    return <section className={styles.ticketSummary} aria-label={`Ticket ${ticket?.number ?? ticketNumber}`}>
      ...
      {ticket && <><select aria-label="Ticket assignee" title="Ticket assignee" value={ticket.assigneeUid ?? ''} disabled={busy} onChange={(event) => void update({ assigneeUid: event.target.value || null })}><option value="">Unassigned</option>{operators.map((operator) => <option key={operator.uid} value={operator.uid}>{operator.displayName || operator.email}</option>)}</select><select aria-label="Ticket priority" title="Ticket priority" value={ticket.priority} disabled={busy} onChange={(event) => void update({ priority: event.target.value as TicketPriority })}><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select><select aria-label="Ticket status" title="Ticket status" value={ticket.status} disabled={busy} onChange={(event) => void update({ status: event.target.value as TicketStatus })}><option value="open">Open</option><option value="in_progress">In progress</option><option value="resolved">Resolved</option><option value="closed">Closed</option></select>{canRetryDelivery && ticket.deliveryState === 'failed' && <button className={styles.iconAction} type="button" onClick={() => void retryDelivery()} disabled={busy} aria-label="Retry external ticket delivery" title="Retry delivery"><RefreshCw size={15} /></button>}{ticket.externalUrl && <a href={ticket.externalUrl} target="_blank" rel="noreferrer" aria-label="Open external ticket" title="Open external ticket"><ExternalLink size={15} /></a>}</>}
  ...
  return <section className={styles.createWrap}>
    {!open ? <button className={styles.createButton} type="button" onClick={() => void begin()} disabled={busy || !agentId}>{busy ? <Loader2 className={styles.spin} size={14} /> : <Plus size={14} />} Create support ticket</button> : <form className={styles.form} onSubmit={create}>
      ...
      {fields.map((field) => <label key={field.id}><span>{field.label}{field.required ? ' *' : ''}</span>{field.type === 'select' ? <select required={field.required} value={String(values[field.id] ?? '')} onChange={(event) => setValues({ ...values, [field.id]: event.target.value })}><option value="">Choose…</option>{field.options?.map((option) => <option key={option}>{option}</option>)}</select> : field.type === 'boolean' ? <select required={field.required} value={values[field.id] === undefined ? '' : String(values[field.id])} onChange={(event) => setValues((current) => { const next = { ...current }; if (!event.target.value) delete next[field.id]; else next[field.id] = event.target.value === 'true'; return next })}><option value="">Choose…</option><option value="true">Yes</option><option value="false">No</option></select> : <input required={field.required} type={field.type === 'number' ? 'number' : 'text'} value={String(values[field.id] ?? '')} onChange={(event) => setValues((current) => { const next = { ...current }; if (field.type === 'number' && !event.target.value) delete next[field.id]; else next[field.id] = field.type === 'number' ? Number(event.target.value) : event.target.value; return next })} />}{field.description && <small>{field.description}</small>}</label>)}
