...
const help: CSSProperties = { margin: '5px 0 0', fontSize: 11, lineHeight: 1.5, color: 'var(--ink-faint)', textWrap: 'pretty' }
...
function ContentSettings({ draft, setDraft, errors, agentId, agentName }: SettingsProps & { agentId: string; agentName: string }) {
  return <>
    <div style={group}><label htmlFor="w-title" style={fieldLabel}>Header title</label><input id="w-title" maxLength={MAX_WIDGET_COPY_CHARS} value={draft.headerTitle} aria-invalid={Boolean(errors.headerTitle)} aria-describedby="w-title-help w-title-error" onChange={(event) => setDraft({ ...draft, headerTitle: event.target.value })} placeholder={agentName} style={{ ...input, padding: '9px 10px', fontSize: 13 }} /><p id="w-title-help" style={help}>Leave blank to use the agent name from <Link href={`/dashboard/agents/${agentId}`} style={{ color: 'var(--accent)' }}>Info settings</Link>.</p><FieldError id="w-title-error">{errors.headerTitle}</FieldError></div>
