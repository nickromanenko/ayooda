...
describe('Inbox helpers', () => {
  ...
  test('derives useful customer labels without exposing channel credentials', () => {
    expect(inboxCustomerIdentity({ visitorId: 'email_jane@example.com' })).toEqual({
      label: 'jane@example.com', email: 'jane@example.com', phone: null, externalId: 'email_jane@example.com', identityTrust: 'anonymous',
    })
    ...
    })).toEqual({ label: 'Ada Lovelace', email: 'ada@example.com', phone: null, externalId: 'customer-42', identityTrust: 'verified' })
