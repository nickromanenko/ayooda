...
- **Threads**: `workspaces/{workspaceId}/copilotUsers/{uid}/threads/{threadId}`, with a `messages` subcollection. The `{uid}` path segment is the entire privacy mechanism (see [Firestore Security Rules](#firestore-security-rules)) — the client reads its own threads directly via `onSnapshot`. The collection is deliberately named `threads`, not `conversations`: the sweep's `collectionGroup('conversations')` queries (`apps/api/src/lib/skills/sweep.ts`) match only that literal collection name, so Copilot threads are structurally invisible to it — never auto-closed, scored, or extracted into visitor memory.
